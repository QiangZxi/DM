...
class ResidualAttentionBlock_MaPLe(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
        ...
        if 'alpha' not in design_details['tsa_opt']:
            self.ad_type = 'none'
        else:
            self.ad_type = design_details['tsa_ad_type']
            self.ad_form = design_details['tsa_ad_form']
        if self.ad_type == 'residual':
            if self.ad_form == 'matrix':
                self.alpha = nn.Parameter(torch.ones(d_model, d_model, 1, 1))
            else:
