...
class CustomCLIP(CustomCLIP_):
    ...
    def _forward_base(self, img, labels=None):
        ...
        logits_lp, labels_lp, _ = self._forward_logits_linear_probe(text_feats, img_feats, labels)
    ...
    def _forward_logits_linear_probe(self, text_feats, img_feats, labels):
        ...
        project = nn.Linear(512, self.len).type(self.dtype).cuda()
        test_logits = project(all_feats)
        all_logits = self.linear_probe_proj(all_feats)
        return all_logits, all_labels, test_logits
    ...
    def _loss(self, logits, labels, logits_lp, labels_lp):
        ...
        return loss, loss_cls, loss_cls_lp
    

@TRAINER_REGISTRY.register()
class ExtrasLinearProbeCoOp_c(CoOp):
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            loss, loss_cls, loss_cls_lp = model(image, label)
            optim1.zero_grad()
            loss_cls.backward(retain_graph=True)
            optim1.step()
            optim2.zero_grad()
            loss_cls_lp.backward()
            optim2.step()
    ...
    """ def model_inference(self, img):
        ...
        logits = self.model._forward_logits_similarity(text_feats, img_feats)
        probabilities = torch.softmax(logits, dim=1)
        pseudo_labels = torch.argmax(probabilities, dim=1)
        # finetune cat head param
        for i in range (50):
            optim.zero_grad()
            _, labels_lp, logits_lp = self.model._forward_logits_linear_probe(text_feats, img_feats, pseudo_labels)
            _, _, loss = self.model._loss(logits, pseudo_labels, logits_lp, labels_lp)
            loss.backward(retain_graph=True)
            optim.step()
        lp_weight = self.model.lp_cfg.WEIGHT
        size = logits_lp.size()
        logits = 0.5 * logits + 0.5 * logits_lp[:size[0]//2,:]
        return logits """
    ...
    def build_model(self):
        ...
        self.optim, self.optim1, self.optim2, infos = build_optimizer(self.model, cfg.OPTIM)
